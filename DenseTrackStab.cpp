...
{	
	...
	while((ptr=readdir(dir)) != NULL){
		if(strcmp(ptr->d_name,".")==0 || strcmp(ptr->d_name,"..")==0)    ///current dir OR parrent dir
        	...
        	char *suffix = ptr->d_name+ strlen(ptr->d_name) - 4;
        	if(strcmp(suffix,".avi")==0){
	        	videofiles.push_back(ptr->d_name);
        	}
	...
	for(int ii = 0; ii < videofiles.size(); ii++){
		char *video =(char*) malloc(strlen(argv[1])+videofiles[ii].size()+6);
		strcpy(video, argv[1]);
		strcat(video,("/"+videofiles[ii]).c_str());
		capture.open(video);
		if(!capture.isOpened()) {
			fprintf(stderr, "%d:%s, Could not initialize capturing..\n", ii, video);
			//return -1;
			free(video);
			continue;
		}
				...
				for (std::list<Track>::iterator iTrack = tracks.begin(); iTrack != tracks.end();) {
					...
					if(iTrack->index >= trackInfo.length) {
						...
						if(IsValid(trajectory, mean_x, mean_y, var_x, var_y, length) && IsCameraMotion(displacement)) {
							...
							PrintDesc(iTrack->hog, hogInfo, trackInfo,out);
							PrintDesc(iTrack->hof, hofInfo, trackInfo,out);
							PrintDesc(iTrack->mbhX, mbhInfo, trackInfo,out);
							PrintDesc(iTrack->mbhY, mbhInfo, trackInfo,out);
							fprintf(out,"\n");
						}
